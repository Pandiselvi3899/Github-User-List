          ...
          user?.login ? (
           ...
           bg-gray-900 p-3 flex-col items-center"
            >
              <img
                src={user?.avatar_url}
                className="w-24 mb-4 border-4  border-green-700 rounded-full"
              />

              <h1 className="text-xl">{user?.login}</h1>
              <h1 className="text-xs text-green-700">{user?.name}</h1>
              <Link to={`/${user?.login}`}>
                <span
                  className="text-gray-200 
                  font-semibold rounded block px-4 py-1 bg-green-700 my-3 tracking-wide"
                >
                  View
                </span>
              </Link>
